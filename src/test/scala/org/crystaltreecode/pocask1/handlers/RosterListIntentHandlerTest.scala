...
class RosterListIntentHandlerTest extends FlatSpec
with Checkers
with MockFactory {
  ...
  "RosterListIntentHandler" should "handle happy path 1" in {
    ...
    val subj = new RosterListIntentHandler()

    val result: Optional[Response] = subj.handle(handlerInput)

    assert(result.isPresent)
    assert(result.get.getOutputSpeech.toString.contains("Your roster for today is:"))
